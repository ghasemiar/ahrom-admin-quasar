<template>
  <q-form @submit="onSubmit" class="md:tw-p-4">
    <div class="tw-grid md:tw-grid-cols-4 tw-m-block-bw tw-gap-4">
      ...
      >
        <q-responsive ratio="1" v-if="fetchedImage">
          <q-img class="tw-relative" :src="getImageUrl(fetchedImage)">
            <div
              class="tw-opacity-0 hover:tw-opacity-100 tw-transition-all tw-duration-300 tw-ease-in-out tw-text-center tw-content-center tw-w-full tw-h-full tw-p-0"
            >
              <q-btn
                :label="$t('remove')"
                class="tw-bg-error-30"
                @click="fetchedImage = null"
              />
            </div>
          </q-img>
        </q-responsive>
        <app-uploader v-else name="image" :model-value="image" label="image" />
...
import {
  ...
  usePostList,
  useUpdatePost,
} from 'src/composables/blog/usePost';
...
//save post method
const { request, loading } = useUpdatePost();
const onSubmit = handleSubmit(async (values) => {
  ...
  !fetchedImage.value && formData.append('image', values.image[0]);
...
watch(getPostData, (newValue) => {
  if (newValue && Object.keys(newValue).length) {
    newValue.image && (fetchedImage.value = newValue.image);
    setFieldValue('content', newValue.content);
    setFieldValue('categories', newValue.categories);
    setFieldValue('title', newValue.title);
    setFieldValue('isPublish', newValue.isPublish);
    setFieldValue('description', newValue.description);
    setFieldValue('slug', newValue.slug);
    setFieldValue('tags', newValue.tags);
    setFieldValue('keywords', newValue.keywords);
    setFieldValue('excerpt', newValue.excerpt);
    setFieldValue(
      'related_post',
      newValue.related_post?.map((post) => post.id)
    );
  }
});
</script>
