...
export const useCategoryList = () => {
  ...
  const items = reactive<{ loading: boolean; data: CategoryListDto }>({
    loading: false,
    data: {} as CategoryListDto,
  });
  const { t } = useI18n();
  const request = () => {
    items.loading = true;
    getCategoryListService(baseRoute)
      .then(async (res) => {
        items.loading = false;
        if (res != undefined) {
          console.log(categoryListTransformer(res));
          items.data = categoryListTransformer(res);
